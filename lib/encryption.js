...
var KeyDerivationAsync = require('./keyderivation_async');
...
Encryption.encryptWithSaltAndIV = function(pt, pw, saltBuf, iv, iterations) {
    return q.when()
        .then(function() {
            ...
            return KeyDerivationAsync.compute(pw, saltBuf, iterations)
                .then(function(keyBuf) {
                    var key = sjcl.codec.hex.toBits(keyBuf.toString('hex'));
...
Encryption.decrypt = function(ct, pw) {
    return q.when()
        .then(function() {
            ...
            return KeyDerivationAsync.compute(pw.slice(0), salt.slice(0), iterations)
                .then(function(keyBuf) {
                    var key = sjcl.codec.hex.toBits(keyBuf.toString('hex'));
