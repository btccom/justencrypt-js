...
Encryption.encryptSync = function(pt, pw, iterations) {
    var salt = this.generateSalt();
    var iv = this.generateIV();

    iterations = typeof iterations === 'undefined' ? KeyDerivation.defaultIterations : iterations;
    return this.encryptWithSaltAndIVSync(pt, pw, salt, iv, iterations);
};

Encryption.encryptWithSaltAndIVSync = function(pt, pw, saltBuf, iv, iterations) {
    ...
    var key = sjcl.codec.hex.toBits(KeyDerivation.computeSync(pw, saltBuf, iterations).toString('hex'));
...
Encryption.decryptSync = function(ct, pw) {
    ...
    var key = KeyDerivation.computeSync(pw, salt, iterations);
